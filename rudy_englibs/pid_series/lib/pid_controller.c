...
static float PID_CONTROLLER_compute_pid(pid_controller_t* pid, float setpoint,
...
float PID_CONTROLLER_compute_pid(pid_controller_t* pid, float setpoint, float measured_value) {
    ...
    float alpha          = pid->kd_N * pid->dt / (1.0f + pid->kd_N * pid->dt);
    float derivative_raw = (measured_value - pid->previous_measurement) / pid->dt;
    pid->filtered_derivative += alpha * (-derivative_raw - pid->filtered_derivative);
    ...
    if (output_unsat > pid->output_max)
        output = pid->output_max;
    if (output_unsat < pid->output_min)
        output = pid->output_min;

    // 4. Anti-windup: Adjust integral term if output is saturated
    if (pid->ki_aw > 0.0f && fabsf(pid->ki) > 1e-6f) {
        float aw_error = (output_unsat - output); // output_unsat > output saat saturasi positif
        pid->integral -= pid->ki_aw * aw_error;   // koreksi integrator langsung
    }
...
void PID_CONTROLLER_init(pid_controller_t* pid, float kp, float ki, float kd, float output_min,
                         float output_max, float dt, float anti_windup_factor,
    ...
    pid->integral_max      = pid->output_max / (pid->ki > 1e-6f ? pid->ki : 1e-6f);
